...
from gmx_python_sdk.scripts.v2.get.get import (
    GetData
)
...
@router.post("/")
def fetch_liquidation_price(
        liq_price_request: LiqRequest,
        config: ConfigManager = Depends(config_manager_dependency)
    ):
    """
    Endpoint to fetch positions using the provided address on GMX.
    """
    try:
        wallet_address = liq_price_request.address
        liq_info = liq_price_request.liq_info

        positions = get_positions(config, address=wallet_address)
        
        liquidation_price = get_liquidation_price(
            config, positions[liq_info], wallet_address
        )
        return { liq_info: liquidation_price }
