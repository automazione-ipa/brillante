...
@router.post("/")
def find_farming(
        find_request: FarmingRequest,
        config: ConfigManager = Depends(config_manager_dependency)
    ):
    """
    Endpoint to find farming opportunities and create an increase order on GMX.
    """
    try:
        order_params = OrderArgumentParser(
        ).process_parameters_dictionary(
            find_request.dict(exclude={"debug_mode"})
        )

        stats = check_if_viable_farming_strategy(
            order_params,
            ignore_oi_imbalance=find_request.ignore_oi_imbalance
        )

        increase_order = IncreaseOrder(
            config=config,
            market_key=order_params['market_key'],
            collateral_address=order_params['start_token_address'],
            index_token_address=order_params['index_token_address'],
            is_long=order_params['is_long'],
            size_delta=order_params['size_delta'],
            initial_collateral_delta_amount=(
                order_params['initial_collateral_delta']
            ),
            slippage_percent=order_params['slippage_percent'],
            swap_path=order_params['swap_path'],
            debug_mode=find_request.debug_mode
        )

        # In debug mode, you might not want to execute the order but just see the parameters.
        return {"status": "success", "order_details": order_params}

    except Exception as e:
        error = 'Position not viable, reason: "{}"'.format(e)
